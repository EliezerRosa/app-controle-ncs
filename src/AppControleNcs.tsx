...
const REPO_NAME = 'app-controle-ncs';
...
const AppControleNcs: React.FC = () => {
  ...
  const persistPat = useCallback((token: string) => {
  ...
  const recoverTokenArtifact = useCallback(async ({ silent = false }: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      setPatDraft(reconstructed);
      console.info('[controle-ncs] Token reconstruído.');
      return true;
    } catch (error) {
  ...
  }, [recoverTokenArtifact, setInitialLoadTriggered, setPatDraft, surfaceNotification]);

  const loadRemoteState = useCallback(
    async (reason: 'initial' | 'manual' | 'polling' | 'focus' | 'post-commit' | 'retry') => {
      if (!IS_DEV_BUILD && !patValue) {
        console.info('[controle-ncs] Aguardando token...');
        if (reason === 'manual' || reason === 'retry') {
          setShowPatModal(true);
          surfaceNotification({ type: 'error', message: 'Configure o PAT antes de sincronizar os dados.' });
        }
        return;
      }
      ...
      try {
        ...
        console.info('[controle-ncs] Dados carregados.');
        surfaceNotification({
          type: 'success',
          message:
            reason === 'initial'
        ...
        if (typedError.code === 'PAT_REQUIRED') {
          surfaceNotification({ type: 'error', message: typedError.message });
          setShowPatModal(true);
        } else {
          surfaceNotification({
            type: 'error',
            message: typedError.message ?? 'Erro ao buscar dados',
          });
        }
      } finally {
        setIsLoading(false);
    ...
    [IS_DEV_BUILD, patValue, selectedTerritoryId, surfaceNotification],
  );
  ...
  useEffect(() => {
    ...
    if (!IS_DEV_BUILD && !patValue) return;
    setInitialLoadTriggered(true);
    void loadRemoteState('initial');
  }, [IS_DEV_BUILD, initialLoadTriggered, loadRemoteState, patValue]);
  ...
  useEffect(() => {
    if (!showPatModal) return;
    setPatDraft(patValue);
  }, [patValue, showPatModal]);
  ...
  const handlePatSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      event.preventDefault();
      const sanitizedToken = patDraft.trim();
      if (!sanitizedToken) return;
      persistPat(sanitizedToken);
      setShowPatModal(false);
      surfaceNotification({ type: 'success', message: 'PAT salvo localmente' });
      void distributeTokenToPages(sanitizedToken);
    },
    [distributeTokenToPages, patDraft, persistPat, surfaceNotification],
  );
  ...
  const patModalElement = showPatModal ? (
  ...
  if (isLoading && !appState) {
    const loadingMessage = patValue
      ? 'Sincronizando com o GitHub...'
      : 'Aguardando token de acesso...';
    return (
